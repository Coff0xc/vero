...
// 输入为 URL/IP 时动态置顶「发起战役」; 普通文本按标题模糊过滤。纯文字, 无图标。
export function CommandPalette() {
  ...
  const cmds = useMemo<Cmd[]>(() => {
    ...
    const list: Cmd[] = [
      { id: 'new', title: '新建会话', run: () => { reset(''); go('campaign')() } },
      ...(status === 'running'
        ? [{ id: 'stop', title: '停止当前战役', run: async () => { await cancelCampaign(); toggle(false) } }]
        : []),
      { id: 't-campaign', title: '转到 对话', hint: 'Ctrl 1', run: go('campaign') },
      { id: 't-tools', title: '转到 工具', hint: 'Ctrl 2', run: go('tools') },
      { id: 't-workflows', title: '转到 工作流', hint: 'Ctrl 3', run: go('workflows') },
      { id: 't-reports', title: '转到 报告', hint: 'Ctrl 4', run: go('reports') },
      { id: 't-settings', title: '转到 设置', hint: 'Ctrl 5', run: go('settings') },
      { id: 'history', title: '历史战役', hint: 'Ctrl H', run: () => { toggle(false); toggleHistory(true) } },
      {
        id: 'graph-full',
    ...
    if (TARGET_RE.test(query)) {
      dyn.push({
        id: 'start', title: `发起战役 ${query}`,
        run: async () => { toggle(false); await startCampaign(query) },
      })
    }
    dyn.push({
      id: 'grep-node', title: `搜索节点 ${query}`,
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-[14vh]" onClick={() => toggle(false)}>
      <div className="w-[min(560px,92vw)] bg-white border border-line rounded-lg shadow-pop panel-in overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2.5 px-4 border-b border-line">
          <input
            ...
            className="flex-1 bg-transparent outline-none py-3.5 text-[13.5px] text-ink2 placeholder:text-ghost/90"
          />
        ...
        <div className="max-h-[46vh] overflow-y-auto py-1">
          ...
          {cmds.map((c, i) => (
            <button
              ...
              onClick={() => void c.run()}
              className={`w-full flex items-center justify-between gap-3 px-4 py-2.5 text-left text-[13px] transition-colors ${
                i === cursor ? 'bg-panel2 text-ink2' : 'text-muted'
              }`}
            >
              <span className="truncate">{c.title}</span>
              {c.hint && <span className="kbd shrink-0">{c.hint}</span>}
            </button>
          ))}
        </div>
        <div className="px-4 py-2 border-t border-line flex items-center gap-3 text-[10px] text-ghost">
