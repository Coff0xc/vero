...
// 命令面板(⌘K)—— 全局动作中枢: 导航 / 战役控制 / 图全屏 / 节点搜索。
...
export function CommandPalette() {
  ...
  const cmds = useMemo<Cmd[]>(() => {
    ...
    const list: Cmd[] = [
      { id: 'new', title: '新会话', icon: '✚', hint: '', run: () => { reset(''); go('campaign')() } },
      ...(status === 'running'
        ? [{ id: 'stop', title: '停止当前战役', icon: '■', hint: '', run: async () => { await cancelCampaign(); toggle(false) } }]
        : []),
      { id: 't-campaign', title: '转到: 对话作战', icon: '💬', hint: 'Ctrl+1', run: go('campaign') },
      { id: 't-tools', title: '转到: 工具管理', icon: '🧰', hint: 'Ctrl+2', run: go('tools') },
      { id: 't-workflows', title: '转到: 工作流', icon: '⚡', hint: 'Ctrl+3', run: go('workflows') },
      { id: 't-reports', title: '转到: 报告', icon: '📄', hint: 'Ctrl+4', run: go('reports') },
      { id: 't-settings', title: '转到: 设置', icon: '⚙', hint: 'Ctrl+5', run: go('settings') },
      { id: 'history', title: '历史战役', icon: '🕘', hint: 'Ctrl+H', run: () => { toggle(false); toggleHistory(true) } },
      {
        id: 'graph-full',
    ...
    if (TARGET_RE.test(query)) {
      dyn.push({
        id: 'start', title: `发起战役: ${query}`, icon: '🎯',
        run: async () => { toggle(false); await startCampaign(query) },
      })
    }
    dyn.push({
      id: 'grep-node', title: `搜索节点: ${query}`, icon: '🔍',
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start justify-center pt-[14vh]" onClick={() => toggle(false)}>
      <div className="w-[min(560px,92vw)] glass border border-line rounded-xl shadow-card panel-in overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2.5 px-4 border-b border-line/70">
          <span className="text-signal text-[14px]">⌘</span>
          <input
            ref={inputRef}
            value={q}
        ...
        <div className="max-h-[46vh] overflow-y-auto py-1.5">
          ...
          {cmds.map((c, i) => (
            ...
            >
              <span className={`text-[14px] ${i === cursor ? 'text-signal' : ''}`}>{c.icon}</span>
